	...
	elRTE.prototype.filter.prototype.rules = {
		...
		clean : function(html) {
			...
			return html.replace(/<!DOCTYPE([\s\S]*)>/gi, '')
                ...
                // Comment Sebastien : correct unwanted <br />
				.replace(/(<p[^>]*>\s*<\/p>|<p[^>]*\/>)/gi, "\n")
			...
			if (!this.rte.options.allowTextNodes) {
				...
				function wrap() {
					if (w.length && dom.filter(w, 'notEmpty').length) {
                        // Comment Sebastien :  deactivate wrapping
...
elRTE.prototype.selection = function(rte) {
	...
	this.getRangeAt = function(updateW3cRange) {
		...
		var s = selection();
		var r = s && s.rangeCount > 0 ? s.getRangeAt(0) : this.rte.doc.createRange();
		r.getStart = function() {
...
elRTE.prototype.ui.prototype.buttons.button.prototype.command = function() {
	...
	try {
		...
		this.rte.doc.execCommand('styleWithCSS', false, !this.rte.options.styleWithCSS);
	} catch (ex) {
		// Use old method
		try {this.rte.doc.execCommand('useCSS', false, !this.rte.options.styleWithCSS);} catch (ex) {}
	}
	try {
...
elRTE.prototype.ui.prototype.buttons.pastetext = function(rte, name) {
	...
	this.paste = function() {
		...
		if (txt) {
			...
			//Comment Sebastien this.rte.selection.insertText(txt.replace(/\r?\n/g, '<br />'), true);
            //this.rte.selection.insertText(txt, true);
			this.rte.ui.update(true);
		}
		this.input.val('');
	}
